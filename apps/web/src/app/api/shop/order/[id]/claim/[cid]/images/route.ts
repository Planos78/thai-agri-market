...
// P6: a shop buyer uploads claim evidence image(s) for their own order's claim. URL-only (bug #7).
export async function POST(req: Request, { params }: { params: Promise<{ id: string; cid: string }> }) {
  ...
  if (order.buyer.phone !== session.phone) return NextResponse.json({ error: "forbidden" }, { status: 403 });
