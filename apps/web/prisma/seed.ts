...
async function main() {
  ...
  if ((await prisma.lot.count({ where: { orchardId: orchard.id } })) === 0) {
    await prisma.lot.createMany({
      data: [
        { orchardId: orchard.id, fruitName: "ทุเรียน", variety: "หมอนทอง", grade: "A", price: 180, quantity: 500, unit: "kg", minOrderQty: 5, status: "ACTIVE" },
        { orchardId: orchard.id, fruitName: "มังคุด", grade: "A", price: 90, quantity: 300, unit: "kg", status: "ACTIVE" },
        { orchardId: orchard.id, fruitName: "เงาะ", grade: "B", price: 60, quantity: 400, unit: "kg", status: "ACTIVE" },
      ],
    });
  }
